...
(function () {
  ...
  function wireSaveButton(){
    // v5: auto-enregistrement + bouton optionnel
  ...
  // ===== Checkbox: afficher/masquer le bouton "Réglages OpenSubtitles" =====
  function findOsSettingsButtons(){
    var btns = [];
    ...
    for(var i=0;i<all.length;i++){
      ...
      if(txt === "Réglages OpenSubtitles" || txt === "Reglages OpenSubtitles"){
        btns.push(el);
        continue;
      }
      if(el.id === "openSubtitlesSettingsBtn" || el.id === "opensubtitlesSettingsBtn"){
        btns.push(el);
        continue;
      }
    }
  ...
  function setOsSettingsButtonsVisible(isVisible){
    var btns = findOsSettingsButtons();
    for(var i=0;i<btns.length;i++){
      btns[i].style.display = isVisible ? "" : "none";
    }
  }
  ...
  function wireOsSettingsToggle(){
    ...
    // par défaut: caché
    cb.checked = false;
    setOsSettingsButtonsVisible(false);
